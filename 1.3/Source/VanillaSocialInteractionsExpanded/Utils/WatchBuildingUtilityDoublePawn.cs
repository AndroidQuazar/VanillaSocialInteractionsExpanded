...
{
	public static class WatchBuildingUtilityDoublePawn
	{
		...
		public static bool TryFindBestWatchCellNear(Thing toWatch, Pawn pawn, Building secondChair, bool desireSit, out IntVec3 result, out Building chair)
		{
			List<int> list = CalculateAllowedDirections(toWatch.def, toWatch.Rotation);
			IntVec3 intVec = IntVec3.Invalid;
			for (int i = 0; i < list.Count; i++)
			{
				...
				for (int j = 0; j < num; j++)
				{
					...
					if (EverPossibleToWatchFrom(intVec2, toWatch.Position, toWatch.Map, bedAllowed: false, toWatch.def) && !intVec2.IsForbidden(pawn) && pawn.CanReserve(intVec2) && pawn.Map.pawnDestinationReservationManager.CanReserve(intVec2, pawn))
					{
						...
						{
							building = intVec2.GetEdifice(pawn.Map);
							if (building != null && building.def.building.isSittable && building.Position.DistanceTo(secondChair.Position) < 2 && pawn.CanReserve(building))
							{
								flag = true;
							}
						}
