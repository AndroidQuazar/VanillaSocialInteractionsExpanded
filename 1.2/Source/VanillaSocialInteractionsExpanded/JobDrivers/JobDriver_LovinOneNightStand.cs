...
{
	public class JobDriver_LovinOneNightStand : JobDriver
	{
		...
		protected override IEnumerable<Toil> MakeNewToils()
		{
			...
			{
				if (Partner.CurJob == null || Partner.CurJob.def != VSIE_DefOf.VSIE_OneStandLovin)
				{
					Job newJob = JobMaker.MakeJob(VSIE_DefOf.VSIE_OneStandLovin, pawn, Bed);
			...
			toil2.FailOn(() => Partner.CurJob == null || Partner.CurJob.def != VSIE_DefOf.VSIE_OneStandLovin);
			...
			{
				Thought_Memory thought_Memory = (Thought_Memory)ThoughtMaker.MakeThought(VSIE_DefOf.VSIE_GotSomeLovin);
